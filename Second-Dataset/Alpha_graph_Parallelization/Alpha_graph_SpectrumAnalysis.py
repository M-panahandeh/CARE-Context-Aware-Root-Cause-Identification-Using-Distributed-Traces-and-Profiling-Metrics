...
def process_file(file_path):
    ...
    #louvain:apply_Louvian
    #Info_map:apply_infomap
    # at_communities = apply_Louvian(at_graph)
    # # #print(communities)
    # nt_communities = apply_Louvian(nt_graph)
   ...
   #####################################################Step 3: Apply PageRank based on communities#####################################################
    ...
    # # # for no community: PageRank_withoutCommunity
    ...
    print("time:",(toc-tic), "memoru usage:", memory_usage, "CPU usage:",cpu_percent)
    ...
    try:
        ...
        for node_id, module_id in modules_at_finest_level.items():
            if module_id not in communities:
                communities[module_id] = [index_to_node[node_id]]
            else:
                communities[module_id].append(index_to_node[node_id])
        return communities
    except Exception as e:
        # Handle any exceptions that may occur during processing
        print(f"An exception occurred: {e}")
...
def global_combinational_Page_rank_louvain(graph, communities):
    #this is for louvian
...
if __name__ == '__main__':
        ...
        with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    root_cause, ranked_ochiai = future.result() #ranked_ochiai is service:score ,rank

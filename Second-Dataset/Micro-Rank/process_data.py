...
def get_pagerank_graph(traces): #edited to return requires data structure according to our data
    operation_operation = {}
    operation_trace = {}
    trace_operation = {}
    pr_trace = {}
    ...
    for index, row in traces.iterrows():
        ...
        # Update operation_operation
        if source not in operation_operation:
            operation_operation[source] = []
        if target not in operation_operation:
            operation_operation[target] = []
        if target not in operation_operation[source]:
            operation_operation[source].append(target)

        # Update operation_trace
        if trace_id not in operation_trace:
            operation_trace[trace_id] = []
        if source not in operation_trace[trace_id]:
            operation_trace[trace_id].append(source)
        if target not in operation_trace[trace_id]:
            operation_trace[trace_id].append(target)

        # Update trace_operation
        if source not in trace_operation:
            trace_operation[source] = []
        if trace_id not in trace_operation[source]:
            trace_operation[source].append(trace_id)

        if target not in trace_operation:
            trace_operation[target] = []
        if trace_id not in trace_operation[target]:
            trace_operation[target].append(trace_id)

        # Update pr_trace
        if trace_id not in pr_trace:
            pr_trace[trace_id] = []
        if source not in pr_trace[trace_id]:
            pr_trace[trace_id].append(source)
        if target not in pr_trace[trace_id]:
            pr_trace[trace_id].append(target)

...
PREDICT_COLUMN = 'predict'  ########change to predict for the main TraceRCA
...
def inject_noise(df, ratio=0):
    ...
    if ratio == 0:
        #comment the following lines for no noise function at all
        for idx in range(len(df)):
...
class ItemsetHandler:
    ...
    def gen_itemsets(
        ...
        predict_column = kwargs.get("predict_column", "predict") ####change to predict for the main vetsoin of traceRCA
...
class TraceRCA:
    def __call__(self, input_file: pd.DataFrame, enable_PRFL=False, **kwargs) -> List[str]:
        predict_column = kwargs.get("predict_column", "predict") # ########change to predict for the main TraceRCA
        source_column = kwargs.get("source_column", "source")
        target_column = kwargs.get("target_column", "target")
        trace_id_column = kwargs.get("trace_id_column", "trace_id")
...
def process_file(input_file):
    output_file = r'D:\MYDESK\MyPhd\--Thesis--\paper3\data\A\microservice\TraceRCA\final\doubleroot'
    min_support_rate=0.1
    enable_prfl=True
    quiet=True
    k=100

    main(input_file, output_file, min_support_rate, quiet, k, enable_prfl)
if __name__ == "__main__":

    # read files input
    directory = r"D:\MYDESK\MyPhd\--Thesis--\paper3\data\A\microservice\doubleroor_NewLabeled-alpha-final"

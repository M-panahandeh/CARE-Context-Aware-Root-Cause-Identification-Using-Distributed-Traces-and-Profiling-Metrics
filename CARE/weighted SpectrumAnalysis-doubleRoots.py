...
def process_file(file_path):
    ...
    ##For drwing communities made by "find_weighted_groups"
    # at_colors = [community_id for node in at_graph.nodes() for community_id, community_data in at_communities.items() if
    #              node in community_data['nodes']]
    # nt_colors = [community_id for node in nt_graph.nodes() for community_id, community_data in nt_communities.items() if
    #              node in community_data['nodes']]
    # print(at_communities)
   ...
   #####################################################Step 3: Apply PageRank based on communities#####################################################
    ...
    at_node_scores=global_combinational_Page_rank2(at_graph,at_communities)
    #for normal traces
    nt_node_scores= global_combinational_Page_rank2(nt_graph,nt_communities)
    #no community
    # at_node_scores = node_Page_rank(at_graph)
    # nt_node_scores= node_Page_rank(nt_graph)
    ...
    return rootcause1,rootcause2,ranked_list_of_candiates  #ranked_ochiai includes service: (score , rank)
...
def global_combinational_Page_rank_louvan(graph, communities):
    #this is for louvian
...
if __name__ == '__main__':
        ...
        # Create a DataFrame
        df = pd.DataFrame(
            columns=['Filename', 'Rank1', 'Score1', 'Number of Services with Equal Ranks1', 'Rank2', 'Score2',
        ...
        with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    root_cause1,root_cause2, ranked_ochiai = future.result() #ranked_ochiai is service:score ,rank
                    if root_cause1 in ranked_ochiai:
                        # get rank of root cause
                        score1, rank1 = ranked_ochiai.get(root_cause1, (None, None))
                        # services with the same rank
                        services_with_same_rank1 = [service for service, (_, r) in ranked_ochiai.items() if
                                                   r == rank1 and service != root_cause1]
                    if root_cause2 in ranked_ochiai:
                        # get rank of root cause
                        score2, rank2 = ranked_ochiai.get(root_cause2, (None, None))
                        # services with the same rank
                        services_with_same_rank2 = [service for service, (_, r) in ranked_ochiai.items() if
                                                       r == rank2 and service != root_cause2]
                        #write result in excel
                        df_temp = pd.DataFrame({
                            'Filename': [file_name],
                            'Rank1': [rank1],
                            'Score1': [score1],
                            'Number of Services with Equal Rank1': [len(services_with_same_rank1)],
                            'Rank2': [rank2],
                            'Score2': [score2],
                            'Number of Services with Equal Rank2': [len(services_with_same_rank2)]
                        })
        ...
        df.to_excel('double-root.xlsx', index=False)
        print("\nResult:")

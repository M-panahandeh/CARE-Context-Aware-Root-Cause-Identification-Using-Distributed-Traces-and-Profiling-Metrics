...
def process_file(file_path):
   ...
   #####################################################Step 3: Apply PageRank based on communities#####################################################
    ...
    at_node_scores=global_combinational_weighted_Betweenness(at_graph,at_communities)
    #for normal traces
    nt_node_scores= global_combinational_weighted_Betweenness(nt_graph,nt_communities)
    ...
    return root_cause,ranked_list_of_candiates  #ranked_ochiai includes service: (score , rank)
...
#calculate node score in the general graph * node rank claculated for graph of communites. developed for huristic community detection
def global_combinational_weighted_Betweenness(graph, communities):
    # draw_graph(graph)
    ...
    # Calculate node-level betweenness centrality
    for u, v, data in graph.edges(data=True):
        original = data['weight']
        data['inv_weight'] = 1 / original if original != 0 else float('inf')  # avoid division by zero

    node_betweenness= nx.betweenness_centrality(graph, weight='inv_weight')
    ...
    # Calculate community-level betweenness centrality
    for u, v, data in community_graph.edges(data=True):
        original = data['weight']
        data['inv_weight'] = 1 / original if original != 0 else float('inf')  # avoid division by zero

    community_betweenness = nx.betweenness_centrality(community_graph, weight='inv_weight')

    # For each group, calculate the score for each node
    combined_scores = {}
    for group_id, group_info in communities.items():
        community_score = community_betweenness.get(group_id, 0)
        for node in group_info['nodes']:
            node_score = node_betweenness.get(node, 0)
...
def heuristic_trace_priorization(traces, graph, node_scores):
    ...
    for trace in traces:
        ...
        alpha_product_value = dict_traces_alpha.get(trace_id, 0)
...
def weighted_count_services(graph,traces_score,traces):
    O_e = {}
    O_n = {}
    for service in graph.nodes():
...
if __name__ == '__main__':
        ...
        with ProcessPoolExecutor(max_workers=multiprocessing.cpu_count()) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_file):
                ...
                try:
                    root_cause, ranked_ochiai = future.result()
        ...
        df.to_excel('betweenness-cont.xlsx', index=False)
        print("\nResult:")
